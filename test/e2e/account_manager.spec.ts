...
describe("Account Manager", () => {
    ...
    it('Deploys real contract', async () => {
        ...
        let account = (await contract.query.getAccount(
            defaultSigner.address
        )).value.unwrapRecursively()!;

        let accountContract = new ContractUser(account as unknown as string, defaultSigner, (await ApiSingleton.getInstance()));
