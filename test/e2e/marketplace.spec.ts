...
async function mintAndList(contract: MarketplaceContract, nft: ArchNFTContract, psp22: PSP22Contract, tokenId: Id, price: number, isNative: boolean = false) {
    ...
    if (!isNative) {
        await expect(
            contract.withSigner(bob).tx.listNftForSale(bob.address, nft.address, tokenId, price, CurrencyBuilder.Custom(psp22.address))
...
describe("Marketplace", () => {
    ...
    describe('PSP22 Currency', () => {
        ...
        it('Buy an NFT from a listing.', async () => {
            ...
            await expect(psp22.withSigner(Signers.Alice).tx.approve(contract.address, PRICE_WITH_FEE)).to.eventually.be.fulfilled;
            ...
            await expect(psp22.query.balanceOf(defaultSigner.address)).to.have.returnNumber(INITIAL_BALANCE + PRICE_WITH_FEE - PRICE);
        });
        ...
        it('Buy multiple NFTs from different listings.', async () => {
            ...
            await expect(psp22.query.balanceOf(defaultSigner.address)).to.have.returnNumber(INITIAL_BALANCE + 3 * (PRICE_WITH_FEE - PRICE));
        });
    ...
    describe('Native currency', () => {
        ...
        it('Buy an NFT from a listing.', async () => {
            ...
            // let balanceBeforeAlice = await getBalance(alice);
            ...
            await expect(contract.withSigner(Signers.Alice).tx.buyNft(0, { value: PRICE_WITH_FEE })).to.eventually.be.fulfilled;
            ...
            expect(balanceAfterCreator.toString()).to.be.equal(balanceBeforeCreator.add(PRICE_WITH_FEE_BN).sub(PRICE_BN).toString());
            // Gas fee is not deterministic
            // expect(balanceAfterAlice.toString()).to.be.equal(balanceBeforeAlice.sub(PRICE_WITH_FEE_BN).toString());
